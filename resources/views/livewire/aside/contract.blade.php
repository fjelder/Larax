<div class="flex" x-data="{ openAsideMenu2: {{ UserHelper::sets()->open_aside_menu }} }">
    <div x-show="openAsideMenu2" x-transition:enter="transition ease-out duration-300"
        x-transition:enter-start="opacity-0 -translate-x-full" x-transition:enter-end="opacity-100 translate-x-0"
        x-transition:leave="transition ease-in duration-300" x-transition:leave-start="opacity-100 translate-x-0"
        x-transition:leave-end="opacity-0 -translate-x-full"
        class="xl:flex flex-col flex-shrink-0 w-48 h-full border-r border-gray-200 xl:w-80 dark:border-gray-800 ">
        <!-- top -->

        <div class="p-5 border-b bg-gray-50 border-primary-800">
            <div class="text-xs text-primary-500 tracking-wider underline underline-offset-8 before:content-['#']">
                Kontrakty
            </div>
            ...
            <div class="relative flex items-center justify-end space-x-2">
                <input type="text"
                    ...
                    wire:keydown.enter="jumpToContract($event.target.value)" />
            ...
            <div class="flex my-3">
                <div class="flex items-center mr-3">
                    <input id="inline-radio" type="radio" value="1" wire:model="checkedStage"
                        wire:click="saveNewStage" name="inline-radio-group"
        ...
        <div class="p-5 text-xs tracking-wider text-gray-400 flex items-center justify-between">
            <p>Lista kontraktów</p>

            <button class="text-xl text-primary-600 {{ $sort == 'desc' ? 'block' : 'hidden' }}"
                wire:click="switchSort('asc')">
                <i class="ph-sort-ascending-bold"></i>
            </button>

            <button class="text-xl text-primary-600 {{ $sort == 'asc' ? 'block' : 'hidden' }}"
                wire:click="switchSort('desc')">
                <i class="ph-sort-descending-bold"></i>
            </button>
    ...
    <button @click="openAsideMenu2 = false; Livewire.emitTo('aside.contract', 'visibleAsideMenu', 0)"
        x-show="openAsideMenu2" class="bg-neutral-100/30 flex items-center">
        <x-heroicon-o-chevron-double-left class="w-6 h-6 text-gray-500" />
    </button>
    <button @click="openAsideMenu2 = true; Livewire.emitTo('aside.contract', 'visibleAsideMenu', 1)"
        x-show="!openAsideMenu2" class="bg-neutral-100/30 flex items-center">
        <x-heroicon-o-chevron-double-right class="w-6 h-6 text-gray-500" />
    </button>
</div>
